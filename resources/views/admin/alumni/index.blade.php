              ...
              <tbody>
                ...
                <tr class="border-b">
                  ...
                  <td class="px-4 py-3">
                    @if ($alumnus->user->photo_profile)
                        <img src="{{ asset('storage/' . $alumnus->user->photo_profile) }}"
                            alt="Photo of {{ $alumnus->user->name }}" class="w-12 h-12 rounded-lg object-cover">
                    @else
                        N/A
                    @endif
                  </td>
                  ...
                  <td class="px-4 py-3 flex gap-2">
                    ...
                    <a href="{{ route('admin.alumni.edit', $alumnus->id) }}" class="p-2 rounded hover:bg-gray-100 border border-gray-300"
                        title="Edit">
                    ...
                    <button type="button"
                            onclick="openDeleteModal({{ $alumnus->id }})"
                            class="p-2 rounded hover:bg-red-100 border border-red-300"
                            title="Delete">
                        <i data-feather="trash-2" class="text-red-300"></i>
                    </button>

                    <form id="delete-form-{{ $alumnus->id }}"
                        action="{{ route('admin.alumni.destroy', $alumnus->id) }}"
                        method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
