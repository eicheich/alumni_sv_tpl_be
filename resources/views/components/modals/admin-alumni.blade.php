...
<div id="addAlumniModal"
     ...
     <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg relative">
        ...
        <!-- FORM -->
        <form action="{{ $modalAction }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
              <!-- FOTO -->
            ...
            <div>
              ...
              <input id="name" name="name" type="text" placeholder="Masukkan nama"
                     class="w-full px-4 py-2 bg-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none @error('name') border border-red-500 @enderror" value="{{ old('name') }}">
                @error('name')
                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
              ...
              <input id="nim" name="nim" type="text" placeholder="cth. J0403xxxxxx"
                     class="w-full px-4 py-2 bg-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none @error('nim') border border-red-500 @enderror" value="{{ old('nim') }}">
                @error('nim')
                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
              ...
              <input id="email" name="email" type="email" placeholder="cth. budiono67@gmail.com"
                     class="w-full px-4 py-2 bg-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none @error('email') border border-red-500 @enderror" value="{{ old('email') }}">
                @error('email')
                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div>
              <label class="block text-sm text-gray-700 mb-1">Program Studi</label>
                @if (isset($major) &&
                    ...
                    <select id="major" name="major_id" class="w-full px-4 py-2 bg-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none @error('major') border border-red-500 @enderror" value="{{ old('major') }}">
                ...
                @else
                    <input class="w-full px-4 py-2 bg-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none @error('major') border border-red-500 @enderror" value="{{ old('major') }}">
                @endif
                @error('major')
                    <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                @enderror
            ...
            <!-- Tombol -->
                <button data-close-modal type="button" class="w-full py-2 bg-gray-200 text-black rounded-lg font-medium hover:opacity-90 transition">Batal</button>
